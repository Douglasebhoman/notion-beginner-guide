...
- Links and subpages (pages nested inside pages)

---

## 4. Adding Content with Blocks

Notion builds pages from **blocks** — individual units of content. Every paragraph, image, heading, and list item is its own block.
...
Let's put this into practice with a real example. Suppose you are planning a job application — here is how you would track it in Notion.
...
**Step 3: Add a notes section below**
Type `/h2` and create a heading called *Notes*. Under it, use a regular text block to jot down anything relevant — a contact name, the job URL, or interview details.

You now have a functional, reusable tracker. Check off tasks as you complete them by clicking the checkbox.
...
Notion makes it straightforward to share a page with someone else — whether for feedback, collaboration, or public viewing.
...
Notion includes pre-built templates so you do not have to start from scratch. Templates are useful when you know what you want to track but do not want to build the structure yourself.

**To use a template:**

1. Click **Templates** in the sidebar
2. Browse by category (Personal, Work, School, etc.)
...
- **Project Tracker** — manage tasks across a project with statuses

You can customise any template after adding it — nothing is locked.
...
- **Databases** — Notion's most powerful feature; lets you build structured, filterable tables of information
- **Linked databases** — display the same database in multiple places with different views
- **Notion AI** — built-in writing assistant that can summarise, translate, and draft content
